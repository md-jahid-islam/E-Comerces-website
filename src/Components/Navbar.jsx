...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="bg-white shadow-lg py-5 fixed w-full top-0 z-50">
        <div className="container mx-auto flex justify-between items-center px-5 md:px-0">
          ...
          {/* Menu Links */}
          <ul
            className={`flex-col md:flex-row md:flex md:gap-6 absolute md:static left-0 top-full w-full md:w-auto md:bg-transparent bg-white text-black font-bold md:items-center transition-all duration-300 ease-in-out transform ${
              isOpen ? 'flex' : 'hidden'
            }`}
          >
            {['Home', 'About', 'Portfolio', 'Blog', 'Services', 'Contact'].map((item, index) => (
              <li key={index} className="relative group text-center md:text-left py-2 md:py-0">
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `text-lg ${
                      isActive ? 'text-[#36C2CE]' : 'text-gray-800'
                ...
                {/* Underline animation */}
                <span
                  className="absolute bottom-0 left-0 w-0 h-1 bg-[#36C2CE] transition-all duration-300 group-hover:w-full"
                ></span>
              </li>
            ))}
          </ul>
